...
import { History, Mail, MessageCircle, ShieldCheck, ArrowRight, Headset } from "lucide-react";
              ...
              {requests.map((request) => (
                <div key={request.id} className="px-6 py-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <StatusBadge status={request.processedRecord?.status ?? "PENDING"} />
                  </div>
                  <p className="mt-2 text-sm text-slate-800">{request.rawMessage.slice(0, 140)}{request.rawMessage.length > 140 ? "..." : ""}</p>
                  ...
                  <p className="mt-1 text-xs text-slate-600">
                    {request.processedRecord
                      ? `${CATEGORY_LABELS[request.processedRecord.category] ?? request.processedRecord.category} • ${STATUS_LABELS[request.processedRecord.status] ?? request.processedRecord.status}`
                      : "Processing..."}
                  </p>
                </div>
              ))}
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <nav className="sticky top-0 z-50 border-b border-slate-200 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2"><div className="rounded-lg bg-blue-600 p-1.5 text-white"><Headset size={20} /></div><span className="text-xl font-bold tracking-tight text-slate-900">ARCVAULT</span></div>
          <div className="flex items-center gap-3">
            {session?.user?.role === "CUSTOMER" || !session?.user ? (<Link href="/submit" className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 transition-colors">Submit a Ticket</Link>) : null}
      ...
      <main>
        <section className="bg-white border-b border-slate-200 pt-20 pb-24"><div className="mx-auto max-w-4xl px-6 text-center"><h1 className="text-5xl font-extrabold tracking-tight text-slate-900">The Standard for <span className="text-blue-600">B2B Support.</span></h1><p className="mt-6 text-xl leading-relaxed text-slate-600">ArcVault was founded in 2021 with a simple mission: to bridge the gap between complex enterprise software and the people who keep it running.</p><div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">{session?.user?.role === "CUSTOMER" || !session?.user ? (<Link href="/submit" className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-200 transition hover:bg-blue-700">Submit a Ticket<ArrowRight size={16} /></Link>) : null}<Link href="/login" className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-400 hover:bg-slate-50">Sign In</Link></div></div></section>
